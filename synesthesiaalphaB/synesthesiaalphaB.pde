...
color getNextDColour(){
  ...
  b = int(random(0,10));
  g = int(random(10,110));
  return color(g+redboost,g,10, 10);
}
...
void mouseMoved(){
 ...
 for(int i = 0; i < letters.length; i++){
   if(hovering(letters[i])){
     LETTER = letters[i];
     for(int j = 0; j < 20; j++){
       sketchyCircle(getNextColour(LETTER), XRANGE, YRANGE);
     }
   } 
 }
}
...
void draw() {
  if(LETTER == 'D'){
    shimmerCircle(getNextColour(LETTER), XRANGE, YRANGE);
  } else if(LETTER != ' '){
     sketchyCircle(getNextColour(LETTER), XRANGE, YRANGE);
  } else {
    mybackground();
...
void shimmerCircle(color c, int XRANGE, int YRANGE){
beginShape();
